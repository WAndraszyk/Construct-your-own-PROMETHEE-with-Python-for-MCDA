...
from .preference_commons import directed_alternatives_performances


def check_dominance_condition(
    criteria_directions: pd.Series, category_profiles: pd.DataFrame
):
    """
...
def check_dominance_condition_GDSS(
    profiles: pd.Index,
            ...
            for i in range(len(profiles) - 1):
                ...
                for DM_j_profiles_performances in profiles_performances:
                    ...
                    if profile_i[criterion] >= profile_j[criterion]:
                        raise ValueError(
                            "Each profile needs to be preferred "
...
def check_if_profiles_are_strictly_worse(
    criteria_thresholds: pd.Series, category_profiles: pd.DataFrame
):
    """
    Check if each boundary profile is strictly worse in each criterion
    ...
    for criterion, threshold in criteria_thresholds.items():
        for i, (_, profile_i) in enumerate(category_profiles.iloc[:-1].iterrows()):
            ...
            if profile_i[criterion] + threshold > profile_j[criterion]:
                raise ValueError(
                    "Each profile needs to be preferred over "
