...
def _check_generalized_criteria(generalized_criteria: pd.Series):
    ...
    if not (generalized_criteria.values.all() in
            [GeneralCriterion.USUAL,
...
def _check_weights(weights: pd.Series, criteria_num: int):
    ...
    if len(weights) != criteria_num:
        raise ValueError(
            "Number of weights should be equals to number of criteria")
...
def _check_if_all_object_have_the_same_criteria(
...
def pclust_validation(alternatives_performances: pd.DataFrame,
    ...
    _check_if_all_object_have_the_same_criteria(alternatives_performances,
                                                preference_thresholds,
                                                indifference_thresholds,
                                                standard_deviations,
                                                generalized_criteria,
...
def ordered_clustering_validation(preferences: pd.DataFrame,
                                  clusters_no: int):
