...
from ..enums import FlowType, ScoringFunction, ScoringFunctionDirection

__all__ = [
    "net_flow_score_validation",
    "promethee_group_ranking_validation",
...
def _check_scoring_function(
    function: ScoringFunction, direction: ScoringFunctionDirection
):
    """
...
def _check_avoid_same_scores(avoid_same_scores: bool):
    ...
    if not isinstance(avoid_same_scores, bool):
        raise ValueError("Avoid same scores parameter should be a boolean " "value")
...
def _check_dms_weights(dms_weights: pd.Series, n_dms: int):
    ...
    if len(dms_weights) != n_dms:
        raise ValueError("Number of DM's weights should be equals " "to number of DM's")
...
def _check_criteria_weights_gdss(
    ...
    if (
        not len(dms_profiles_partial_preferences[0].index.get_level_values(0).unique())
        == len(
            dms_alternatives_partial_preferences[0].index.get_level_values(0).unique()
        )
        == len(
...
def _check_dms_profile_vs_profile_partial_preferences(
    ...
    number_of_criteria = len(
        dms_profile_vs_profile_partial_preferences.index.get_level_values(0).unique()
    )
    ...
    profiles_index = dms_profile_vs_profile_partial_preferences.index.droplevel(
        0
    ).unique()
    ...
    for (
        index,
        profile_vs_profile_partial_preferences,
    ) in dms_profile_vs_profile_partial_preferences.groupby(level=0):
        if not (
            profile_vs_profile_partial_preferences.droplevel(0).index.equals(
                profiles_index
            )
            and (profile_vs_profile_partial_preferences.columns.equals(profiles_index))
        ):
            raise ValueError(
                "DMs profile vs profile partial preferences should be passed "
                "as a DataFrame with "
...
def _check_number_of_dms_gdss(
    ...
    if not (
        len(dms_profiles_partial_preferences[0].index.get_level_values(0).unique())
        == len(
            dms_alternatives_partial_preferences[0].index.get_level_values(0).unique()
        )
        == len(
        ...
        raise ValueError(
            "Number of DMs should be the same in every DMs" " partial preferences"
        )


def _check_dms_alternatives_partial_preferences(
    alternatives_partial_prefs: List[pd.DataFrame],
):
    """
    Check if DMs alternatives partial preferences are valid in FlowSortGDSS
    method.

    :param alternatives_partial_prefs: List with pd.DataFrame
    ...
    if not isinstance(alternatives_partial_prefs, list):
        raise ValueError(
    ...
    if not all(
        isinstance(dms_alternatives_partial_pref, pd.DataFrame)
        for dms_alternatives_partial_pref in alternatives_partial_prefs
    ):
        raise ValueError(
            "DMS alternatives partial preferences should be passed "
            "as a list of DataFrames"
    ...
    # number of alternatives
    if not all(
        len(dms_alternatives_partial_preference) == len(alternatives_partial_prefs[0])
        for dms_alternatives_partial_preference in alternatives_partial_prefs
    ):
        raise ValueError(
    ...
    # number of criteria
    if not all(
        len(dms_alternatives_partial_preference.columns)
        == len(alternatives_partial_prefs[0].columns)
        for dms_alternatives_partial_preference in alternatives_partial_prefs
    ):
        raise ValueError(
    ...
    if not all(
        dms_alternatives_partial_preference.dtypes.values.all()
...
def _check_dms_profiles_partial_preferences(
    ...
    if not all(
        isinstance(dms_profiles_partial_preference, pd.DataFrame)
    ...
    if not all(
        len(dms_profiles_partial_preference) == len(dms_profiles_partial_preferences[0])
        for dms_profiles_partial_preference in dms_profiles_partial_preferences
    ):
        raise ValueError(
    ...
    if not all(
        len(dms_profiles_partial_preference.columns)
...
def net_flow_score_validation(
    alternative_preferences: pd.DataFrame,
...
def check_outranking_flows_type(flow_type: FlowType):
    ...
    if flow_type not in [FlowType.BASIC, FlowType.PROFILE_BASED]:
        raise ValueError("Flow type should be either BASIC or PROFILE_BASED")
