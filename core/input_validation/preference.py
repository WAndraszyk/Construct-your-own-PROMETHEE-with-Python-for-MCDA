...
from ..enums import Direction, GeneralCriterion, InteractionType

__all__ = [
    "promethee_preference_validation",
...
def _check_performances_with_criteria(performances: pd.DataFrame, criteria: pd.Index):
    """
...
def _check_indifference_thresholds(
    indifference_thresholds: pd.Series, criteria: pd.Index
):
    """
    ...
    if not isinstance(indifference_thresholds, pd.Series):
        raise TypeError(
            "Indifference thresholds should be passed as a " "Series object"
        )
    ...
    for threshold in indifference_thresholds:
        if not (
            ...
            raise TypeError(
                "Indifference thresholds should be numeric values" " or None"
            )
...
def _check_standard_deviations(standard_deviations: pd.Series, criteria: pd.Index):
    ...
    if not isinstance(standard_deviations, pd.Series):
        raise TypeError("Standard deviations should be passed as a " "Series object")
    ...
    for deviation in standard_deviations:
        if not (
            isinstance(deviation, int)
            or isinstance(deviation, float)
            or deviation is None
        ):
            raise TypeError("Standard deviations should be numeric values" " or None")
...
def _check_veto_thresholds(veto_thresholds: pd.Series, criteria: pd.Index):
    ...
    if not isinstance(veto_thresholds, pd.Series):
        raise TypeError("Veto thresholds should be passed as a " "Series object")
    ...
    for threshold in veto_thresholds:
        if not (
            ...
            raise TypeError("Veto thresholds should be numeric values" " or None")
...
def _check_generalized_criteria(generalized_criteria: pd.Series, criteria: pd.Index):
    ...
    if not isinstance(generalized_criteria, pd.Series):
        raise TypeError("Generalized criteria should be passed as a " "Series object")
    ...
    if not all(
        isinstance(criterion, GeneralCriterion) for criterion in generalized_criteria
    ):
        raise TypeError("Generalized criteria should be GeneralizedCriteria " "enums")
...
def _check_interactions_types(interactions: pd.Series):
    ...
    if not all([isinstance(value, InteractionType) for value in interactions.values]):
        raise ValueError("Interaction types should be " "of InteractionType enum")
    ...
    for _, row in interactions.iterrows():
        if row["type"] is InteractionType.WKN:
            if row["coefficient"] > 0:
                raise TypeError("Mutual weakening coefficient must be" " less than 0")
...
def _check_reinforced_preference_thresholds(
    ...
    if not isinstance(reinforced_preference_thresholds, pd.Series):
        raise TypeError(
            "Reinforced preference thresholds should" " be passed as a Series object"
        )

    _check_if_criteria_are_the_same(reinforced_preference_thresholds.index, criteria)
    ...
    for threshold in reinforced_preference_thresholds:
        if not (
            ...
            raise TypeError(
                "Reinforced preference thresholds" " should be numeric values or None"
            )

    # Check if reinforced preference thresholds are not negative
    if (reinforced_preference_thresholds < 0).any():
        raise ValueError("Reinforced preference thresholds" " can not be lower than 0")
...
def _check_reinforcement_factors(reinforcement_factors: pd.Series, criteria: pd.Index):
    ...
    if not isinstance(reinforcement_factors, pd.Series):
        raise TypeError("Reinforcement factors should be passed as a Series object")
    ...
    for factor in reinforcement_factors:
        if not (isinstance(factor, int) or isinstance(factor, float) or factor is None):
            raise ValueError(
                "Reinforcement factors should be a numeric " "values or None"
            )
...
def _check_tau(tau: Union[int, float], criteria: List[str]):
    """
...
def _check_preferences(
    ...
    if isinstance(preferences, tuple):
        ...
        for preference_table in preferences:
            ...
            if not preference_table.dtypes.values.all() in [
                "int32",
                "int64",
                "float32",
                "float64",
        ...
        if not (
            preferences[0].index.equals(preferences[1].columns)
            and preferences[0].columns.equals(preferences[1].index)
        ):
            raise ValueError(
                "Preferences should have opposite indexes"
        ...
        if not isinstance(preferences, pd.DataFrame):
            raise ValueError("Preferences should be passed as a DataFrame " "object")

        # Check if preferences dataframe has only numeric values
        if not preferences.dtypes.values.all() in [
            "int32",
            "int64",
            "float32",
            "float64",
        ...
        if not preferences.index.equals(preferences.columns):
            raise ValueError(
                "Preferences should have alternatives as index " "and columns"
            )
...
def promethee_preference_validation(
    ...
    profiles_performance: Union[pd.DataFrame, None],
    decimal_place: int,
):
    """
...
def discordance_validation(
    criteria: List[str],
    partial_preferences: Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]],
    tau: Union[int, float],
    decimal_place: int,
    preferences: Optional[Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]]],
