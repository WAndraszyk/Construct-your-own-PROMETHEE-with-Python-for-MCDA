...
from ..enums import CompareProfiles, Direction

__all__ = [
    "group_class_acceptabilities_validation",
    "prom_sort_validation",
    "promethee_tri_validation",
    "flow_sort_i_validation",
    "flow_sort_ii_validation",
...
def _check_partial_preferences(
    ...
    if isinstance(partial_preferences, tuple):
        ...
        if partial_preferences[0].index.equals(
            partial_preferences[1].columns
        ) or partial_preferences[1].index.equals(partial_preferences[0].columns):
            raise ValueError(
                "Partial preferences for "
        ...
        if not partial_preferences.dtypes.values.all() in [
            "int32",
            "int64",
            "float32",
            "float64",
        ...
        if (
            not partial_preferences.index.get_level_values(1)
            .unique()
            .equals(partial_preferences.columns)
            and not with_profiles
        ):
            raise ValueError(
        ...
        n_alternatives = [
            len(criterion_preferences.index)
            for _, criterion_preferences in partial_preferences.groupby(level=0)
        ]
...
def _check_assignments(assignments: List[pd.DataFrame], categories: List[str]):
    ...
    if not isinstance(assignments, List):
        raise ValueError(
            "Assignments should be passed as a " "List of DataFrame objects"
        )
    ...
    for assignment in assignments:
        ...
        if not isinstance(assignment, pd.DataFrame):
            raise ValueError(
                "Each assignment of DM should be " "passed as a DataFrame object"
            )

        # Check if assignments dataframe have 'better' and 'worse' columns
        if "worse" not in assignment.columns or "better" not in assignment.columns:
            raise ValueError(
                "Columns of DataFrame with assignments"
        ...
        for worse_cat, better_cat in zip(assignment["worse"], assignment["better"]):
            if worse_cat not in categories or better_cat not in categories:
                raise ValueError(
                    "Alternative can not be assign to category"
                    " that does not exist in categories list"
                )


def _check_if_criteria_are_the_same(criteria_1: pd.Index, criteria_2: pd.Index):
    """
...
def _check_preference_thresholds(preference_thresholds: pd.Series, criteria: pd.Index):
    ...
    if not isinstance(preference_thresholds, pd.Series):
        raise TypeError("Preference thresholds should be passed as " "a Series object")
    ...
    for threshold in preference_thresholds:
        if not (
            ...
            raise TypeError("Preference thresholds should be numeric values" " or None")
...
def _check_performances(performances: pd.DataFrame):
    ...
    if not isinstance(performances, pd.DataFrame):
        raise ValueError("Performances should be passed " "as a DataFrame")

    # Check if performances are numeric
    if not performances.dtypes.values.all() in [
        "int32",
        "int64",
        "float32",
        "float64",
    ]:
        raise ValueError("Performances should be passed " "with int or float values")
...
def _check_criteria_directions(criteria_directions: pd.Series):
    ...
    if not np.isin(criteria_directions.values, [Direction.MAX, Direction.MIN]).all():
        raise ValueError(
            "Criteria directions should be " "prometheePUT_core.enums.Direction enums"
        )
...
def _check_assign_to_better(assign_to_better: bool):
    ...
    if not isinstance(assign_to_better, bool):
        raise ValueError(
            "Assign to better class parameter should" " have value True or False"
        )
...
def _check_marginal_val(use_marginal_val: bool):
    ...
    if not isinstance(use_marginal_val, bool):
        raise ValueError(
            "Use marginal value parameter should " "have value True or False"
        )
...
def _check_compare_profiles(compare_profiles: CompareProfiles):
    ...
    if not isinstance(compare_profiles, CompareProfiles):
        raise TypeError(
            "Compare profiles parameter should be "
            "prometheePUT_core.enums.CompareProfiles enum"
        )
...
def _check_compare_profiles_type_and_profiles_and_categories_length(
    ...
    if compare_profiles is CompareProfiles.LIMITING_PROFILES:
        ...
        if len(categories) != len(category_profiles):
            raise ValueError(
                "Number of categories should be equals to number of " "central profiles"
            )


def _check_profile_based_flows(
    profile_based_flows: pd.DataFrame, category_profiles: pd.DataFrame
):
    """
    ...
    if not isinstance(profile_based_flows, pd.DataFrame):
        raise ValueError("Profile-based flows should be passed as a DataFrame object")

    # Check if profile-based flows have MultiIndex
    if not isinstance(profile_based_flows.index, pd.MultiIndex):
        raise ValueError(
            "Profile-based flows should be passed as a DataFrame with " "MultiIndex"
        )
    ...
    for index, profile_based_alternative_flows in profile_based_flows.groupby(level=0):
        if not len(profile_based_alternative_flows) == len(category_profiles) + 1:
            raise ValueError(
...
def _check_net_flows(net_flows: pd.Series):
    ...
    if net_flows.dtype not in ["int32", "int64", "float32", "float64"]:
        raise ValueError("Net flows should be passed as a Series with numeric values")
...
def _check_profiles_general_net_flows(
    ...
    if not isinstance(profiles_general_net_flows, pd.DataFrame):
        raise ValueError(
            "Profiles general net flows should be passed as a " "DataFrame object"
        )
    ...
    if not (
        profiles_general_net_flows.columns == alternatives_general_net_flows.index
    ).all():
        raise ValueError(
    ...
    for (
        index,
        profiles_general_net_flows_group,
    ) in profiles_general_net_flows.groupby(level=0):
    ...
    if profiles_general_net_flows.dtypes.values.any() not in [
        "int32",
        "int64",
        "float32",
        "float64",
...
def _check_profiles_performances_list(
    ...
    if not all(
        isinstance(profiles_performance, pd.DataFrame)
    ...
    if not all(
        len(profiles_performance) == len(profiles_performances[0])
        for profiles_performance in profiles_performances
    ):
        raise ValueError(
            "Number of profiles in every profiles performances " "should be the same"
        )

    # Check if profiles performances have the same number of criteria
    if not all(
        len(profiles_performance.columns) == len(profiles_performances[0].columns)
        for profiles_performance in profiles_performances
    ):
        raise ValueError(
            "Number of criteria in every profiles performances " "should be the same"
        )

    # Check if profiles performances have numeric values
    if not all(
        profiles_performance.dtypes.values.all()
...
def _check_dms_weights(
    dms_weights: pd.Series, profiles_general_net_flows: pd.DataFrame
):
    """
    ...
    if not (
        dms_weights.index
        == profiles_general_net_flows.index.get_level_values(0).unique()
    ).all():
        raise ValueError(
...
def _check_number_of_dms(
    ...
    if (
        not len(profiles_general_net_flows.index.get_level_values(0).unique())
        == len(profiles_performances)
        == len(dms_weights.index)
    ):
        raise ValueError(
...
def promethee_tri_validation(
    ...
    """
    criteria_from_df = (
        alternatives_partial_preferences[0].index.get_level_values(0).unique()
    )
...
def flow_sort_i_validation(
    ...
    _check_if_criteria_are_the_same(criteria_from_df, criteria_directions.index)
    _check_categories(categories)
    _check_performances(category_profiles)
    _check_criteria_directions(criteria_directions)
    _check_flows(alternatives_flows)
...
def flow_sort_ii_validation(
    ...
    _check_compare_profiles_type_and_profiles_and_categories_length(
        comparison_with_profiles, categories, category_profiles
    )
    _check_if_criteria_are_the_same(
        category_profiles.columns, criteria_directions.index
    )


def flow_sort_gdss_validation(
    alternatives_general_net_flows: pd.Series,
    profiles_general_net_flows: pd.DataFrame,
    categories: List[str],
    criteria_directions: pd.Series,
    ...
    _check_profiles_general_net_flows(
    ...
    _check_compare_profiles(comparison_with_profiles)
    _check_compare_profiles_type_and_profiles_and_categories_length(
        comparison_with_profiles, categories, profiles_performances[0], True
    )
    _check_assign_to_better(assign_to_better_class)


def group_class_acceptabilities_validation(
