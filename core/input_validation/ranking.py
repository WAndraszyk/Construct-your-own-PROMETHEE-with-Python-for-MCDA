...
from ..enums import ScoringFunction, ScoringFunctionDirection
from .flow import net_flow_score_validation

__all__ = [
    "promethee_i_ranking_validation",
...
def _check_weak_preference(weak_preference: bool):
    ...
    if not isinstance(weak_preference, bool):
        raise ValueError("Weak preference parameter should" " be a boolean value")


def _check_net_flowsII(net_flows: Union[pd.DataFrame, pd.Series]):
    """
    Check if net flows II are valid.

    :param net_flows: pd.DataFrame with alternatives as index and 'net' column
     at least or pd.Series with alternatives as index and flows as values
    :raise ValueError: if any net flow is not valid
    """

    # Check if net flows are passed as a Series
    if isinstance(net_flows, pd.Series):
        # Check if net flows are numeric values
        if net_flows.dtype not in ["int32", "int64", "float32", "float64"]:
            raise ValueError("Net Flow should be passed with numeric values")
    else:
        # Check if net flows are passed as a DataFrame
        if not isinstance(net_flows, pd.DataFrame):
            raise TypeError("Net Flows should be passed as a DataFrame object")

        # Check if net flows dataframe have 'net' column
        if "net" not in net_flows.columns:
            raise ValueError('No "net" column in given flows')

        # Check if net columns has numeric values
        if net_flows["net"].dtype not in [
            "int32",
            "int64",
            "float32",
            "float64",
...
def _check_alpha(alpha: Union[int, float]):
...
def promethee_ii_ranking_validation(net_flow: Union[pd.DataFrame, pd.Series]):
    """
    Check if all inputs are valid for Promethee II ranking.

    :param net_flow: pd.DataFrame with alternatives as index and 'net' column
    at least or pd.Series with alternatives as index and flows as values
...
def net_flow_score_iterative_validation(
    ...
    :raise ValueError: if any input is not valid
    """
    net_flow_score_validation(alternative_preferences, function, direction, True)
...
def promethee_iii_ranking_validation(
    flows: pd.DataFrame,
