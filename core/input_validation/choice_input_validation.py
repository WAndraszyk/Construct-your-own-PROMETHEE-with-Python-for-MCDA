...
def _check_constraint(constraint: Constraint):
    ...
    if not isinstance(constraint.b, (int, float)):
        raise TypeError("Right side of the condition should be passed as a numeric value")


def _check_constraints(constraints: List[Constraint]):
    if not isinstance(constraints, list):
        raise TypeError("Constraints should be passed as a list of Constraint objects")
    else:
