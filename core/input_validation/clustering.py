...
def _check_alternatives_performances(alternatives_performances: pd.DataFrame):
    ...
    if not alternatives_performances.dtypes.values.all() in [
        "int32",
        "int64",
        "float32",
        "float64",
        ...
        raise ValueError(
            "Alternatives performances should be passed " "with int or float values"
        )
...
def _check_generalized_criteria(generalized_criteria: pd.Series):
    ...
    if not all(
        ...
        raise ValueError(
            "Generalized criteria should be "
            "prometheePUT_core.enums.PreferenceFunction enums"
        )
...
def _check_preference_thresholds_better_than_indifference_thresholds(
    preference_thresholds: pd.Series, indifference_thresholds: pd.Series
):
    """
    ...
    # Check if preference thresholds are better than indifference thresholds
    if (
        preference_thresholds[preference_thresholds < indifference_thresholds] != 0
    ).any():
        raise ValueError(
            "Preference thresholds should be greater than " "indifference thresholds"
        )
...
def _check_if_all_inputs_have_the_same_criteria(
    ...
    tmp_criteria = alternatives_performances.columns
    # Check if all inputs have the same criteria
    if not (
        tmp_criteria.equals(preference_thresholds.index)
        and tmp_criteria.equals(indifference_thresholds.index)
        and tmp_criteria.equals(standard_deviations.index)
        and tmp_criteria.equals(generalized_criteria.index)
        and tmp_criteria.equals(criteria_directions.index)
        and tmp_criteria.equals(criteria_weights.index)
    ):
        raise ValueError("All objects should have the same criteria")


def _check_n_clusters(alternatives_performances: pd.DataFrame, n_clusters: int):
    """
    ...
    if alternatives_performances.shape[0] <= n_clusters:
        raise ValueError(
            "Number of categories should be lower than number " "of alternatives"
        )
...
def promethee_cluster_validation(
    alternatives_performances: pd.DataFrame,
    preference_thresholds: pd.Series,
    indifference_thresholds: pd.Series,
    standard_deviations: pd.Series,
...
def intervalp2clust_validation(
    alternatives_performances: pd.DataFrame,
    preference_thresholds: pd.Series,
    indifference_thresholds: pd.Series,
    standard_deviations: pd.Series,
    ...
    _check_preference_thresholds(preference_thresholds, criteria_weights.index)
    _check_indifference_thresholds(indifference_thresholds, criteria_weights.index)
    _check_standard_deviations(standard_deviations, criteria_weights.index)
    ...
    _check_if_all_inputs_have_the_same_criteria(
        alternatives_performances,
        preference_thresholds,
        indifference_thresholds,
        standard_deviations,
...
def ordered_clustering_validation(
    alternatives_performances: pd.DataFrame, n_clusters: int
):
    """
