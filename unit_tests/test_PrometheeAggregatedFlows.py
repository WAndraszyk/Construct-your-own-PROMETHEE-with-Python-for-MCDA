...
from modular_parts.M11_PrometheeAggregatedFlows import (
    calculate_promethee_aggregated_flows,
)
...
def dms_flows():
    ...
    return pd.DataFrame(
        {
            "DM1": [-0.40, 0.23, 0.06, -0.19, 0.28, 0.02],
...
def test_promethee_group_ranking(dms_flows, dms_weights):
    alternatives = ["a1", "a2", "a3", "a4", "a5", "a6"]
    expected = pd.Series([-0.18, 0.98, 0.57, -0.1, -0.89, -0.42], index=alternatives)
