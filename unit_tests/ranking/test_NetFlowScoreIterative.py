...
def test_net_flow_score_sum_favor(alternatives_preferences):
    alternatives = ['a1', 'a2', 'a3', 'a4', 'a5', 'a6']
    net_flow_score = pd.Series([0.20756, 0.60941, 1.34624, 3.04672, 1.30291, 0.1826], index=alternatives)
    expected = net_flow_score.sort_values(ascending=False)
    actual = calculate_netflow_score_ranking(alternatives_preferences, ScoringFunction.SUM,
...
def test_net_flow_score_min_against(alternatives_preferences):
    alternatives = ['a1', 'a2', 'a3', 'a4', 'a5', 'a6']
    net_flow_score = pd.Series([-0.12798, -0.02185, -0.03683, -0.01092, -0.04697, -0.04370], index=alternatives)
    expected = net_flow_score.sort_values(ascending=False)
    actual = calculate_netflow_score_ranking(alternatives_preferences, ScoringFunction.MIN,
...
def test_net_flow_score_max_difference(alternatives_preferences):
    alternatives = ['a1', 'a2', 'a3', 'a4', 'a5', 'a6']
    net_flow_score = pd.Series([-0.08428, 0.29769, 0.53675, 0.84706, 0.52386, 0.08428], index=alternatives)
    expected = net_flow_score.sort_values(ascending=False)
    actual = calculate_netflow_score_ranking(alternatives_preferences, ScoringFunction.MAX,
