...
from modular_parts.M26_PrometheeIIOrderedClustering import (
    promethee_II_ordered_clustering,
)
...
def criteria_weights():
    criteria = [f"g{i}" for i in range(1, 7)]
    return pd.Series([0.167] * 6, index=criteria)
...
def test_cluster_using_promethee_cluster(
    ...
    assignment = promethee_II_ordered_clustering(
        alternatives_performances,
        preference_thresholds,
        indifference_thresholds,
        standard_deviations,
