...
def alternatives_flows():
    ...
    return pd.DataFrame(
        {'positive': [0.557053009883198, 0.439676427229803, 0.441898772087451,
                      0.14240790655885,
                      0.583432042683531, 0.383647798742138, 0.400119796346211,
                      0.68331224633732,
                      0.40251572327044, 0.0471698113207547, 0.188679245283019,
                      0.739916019922226],
         'negative': [0.144953578915843, 0.30398322851153, 0.289308176100629,
                      0.584007187780773,
                      0.251572327044025, 0.399221323748186, 0.30188679245283,
                      0.0640910452231207,
                      0.352688997142961, 0.768693354515415, 0.632973944293259,
                      0.0]
         }, index=alternatives)
...
def category_profiles_performances():
    ...
    criteria = [f"c{i}" for i in range(1, 6)]
    return pd.DataFrame(
        [[65, 70, 25, 25, 65], [80, 80, 18, 15, 75], [90, 90, 8, 7, 90]],
...
def test_calculate_promsort_sorted_alternatives(
        categories,
        alternatives_flows,
    ...
    expected_first_assigments = pd.DataFrame(
        {'worse': ['C3', 'C3', 'C3', 'C2', 'C3', 'C3', 'C3', 'C4', 'C3', 'C1',
                   'C2', 'C4'],
         'better': ['C3', 'C3', 'C3', 'C2', 'C3', 'C3', 'C3', 'C4', 'C3',
                    'C1', 'C2', 'C4']}, index=alternatives)
    expected_final_assigments = pd.Series(
        ['C3', 'C3', 'C3', 'C2', 'C3', 'C3', 'C3', 'C4', 'C3', 'C1', 'C2',
         'C4'],
        index=alternatives)

    actual_first_assigments, actual_final_assigments = \
        calculate_promsort_sorted_alternatives(
            categories, alternatives_flows,
            category_profiles_flows,
            criteria_thresholds,
            category_profiles_performances,
            criteria_directions,
...
if __name__ == '__main__':
    test_calculate_promsort_sorted_alternatives(
        categories,
        alternatives_flows,
        category_profiles_flows,
        criteria_thresholds,
