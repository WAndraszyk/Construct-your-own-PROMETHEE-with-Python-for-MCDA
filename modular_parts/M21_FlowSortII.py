"""
    .. image:: prometheePUT_figures/M21.jpg
...
def _limiting_profiles_sorting(
    ...
    :param classification: pd.DataFrame with alternatives as index and
        'positive', 'negative' and 'net' as columns. This object is modified
...
def _boundary_profiles_sorting(
    alternative: str,
    alternative_flows: pd.Series,
    profiles_flows: pd.Series,
    categories: List[str],
    ...
    for i, (profile, profile_flows) in enumerate(profiles_flows.iterrows()):
        ...
        if i == (len(profiles_flows) - 1):
            if (
                ...
                > (profile_flows[flow_type] + profiles_flows.iloc[i - 1][flow_type]) / 2
                and flow_type in ["positive", "net"]
            ) or (
                alternative_flows[flow_type]
                < (profile_flows[flow_type] + profiles_flows.iloc[i - 1][flow_type]) / 2
            ...
            if (
                alternative_flows[flow_type]
                <= (profile_flows[flow_type] + profiles_flows.iloc[i + 1][flow_type])
                / 2
                and flow_type in ["positive", "net"]
            ) or (
                alternative_flows[flow_type]
...
def calculate_flowsortII_sorted_alternatives(
    ...
    check_dominance_condition(criteria_directions, profiles_performances)
