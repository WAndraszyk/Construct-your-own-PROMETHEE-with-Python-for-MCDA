...
def _limiting_profiles_sorting(categories: List[str],
    ...
    limiting profiles and assign them to correct class.
    ...
    def _assign_alternative_to_class(alternative: str,
                                     alternative_flows: pd.Series,
                                     profiles_flows: pd.Series,
                                     categories: List[str], flow_type: str):

        for i, (profile, profile_flows) in \
                enumerate(profiles_flows.iterrows()):
            if (alternative_flows[flow_type] <=
                profile_flows[flow_type] and flow_type in
                ['positive', 'net']) or \
                    (alternative_flows[flow_type] >=
                     profile_flows[flow_type] and flow_type == 'negative'):
                if i == 0:
    ...
    for Ralternative, alternative_group_flows in \
        ...
        for flow_type in ['positive', 'negative', 'net']:
            _assign_alternative_to_class(alternative, alternative_flows,
...
def _boundary_profiles_sorting(categories: List[str],
    ...
    """
    Comparing positive and negative flows of each alternative with all
    boundary profiles and assign them to correct class.
    ...
    def _assign_alternative_to_class(alternative: str,
        ...
        for i, (profile, profile_flows) in enumerate(
                profiles_flows.iterrows()):
            if (alternative_flows[flow_type] <= profile_flows[flow_type] and
                flow_type in ['positive', 'net']) or \
                    (alternative_flows[flow_type] >= profile_flows[flow_type]
                     and flow_type == 'negative'):
...
def _central_profiles_sorting(categories: List[str],
    ...
    """
    Comparing positive and negative flows of each alternative with
    all central profiles and assign them to correct class.
    ...
    def _assign_alternative_to_class(
            alternative: str, alternative_flows: pd.Series,
        ...
        for i, (profile, profile_flows) in enumerate(
            ...
            if i == (len(profiles_flows) - 1):
                if (alternative_flows[flow_type] >
                    (profile_flows[flow_type] + profiles_flows.iloc[i - 1][
                        flow_type]) / 2 and
                    flow_type in ['positive', 'net']) or \
                        (alternative_flows[flow_type] <
...
def calculate_flowsortII_sorted_alternatives(
    ...
    :return: DataFrame with altenatives sorted to proper categories using
    flows (positive, negative and net)
    """
    flow_sort_ii_validation(categories, category_profiles,
