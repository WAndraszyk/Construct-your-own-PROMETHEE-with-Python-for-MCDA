...
def _classify_alternative(categories: List[str], classification: pd.DataFrame,
    ...
    """
    Classify (or no) alternative to proper category. Used in first step of
     assignment.

    :param categories: List with categories names (strings only)
    :param classification: DataFrame with already done imprecise assignments
     (columns: 'worse', 'better')
    :param not_classified: Dictionary with alternatives which are not
    precisely classified yet
    :param alternative_assignment: Series with alternative assignment
    of each DM

    :return: Tuple with updated classification(DataFrame) and not_classified
     Dictionary
    """
    if alternative_assignment.nunique() > 1:
        worse_category, better_category = sorted(
            alternative_assignment.unique(),
            key=lambda x: categories.index(x))

        DMs_that_chose_worse_category = alternative_assignment[
            alternative_assignment == worse_category].index
        DMs_that_chose_better_category = alternative_assignment[
            alternative_assignment == better_category].index
        ...
        classification.loc[
            alternative_assignment.name, 'worse'] = worse_category
        classification.loc[
            alternative_assignment.name, 'better'] = better_category
    else:
        classification.loc[alternative_assignment.name, 'worse'] = \
            alternative_assignment.unique()[0]
        classification.loc[alternative_assignment.name, 'better'] = \
            alternative_assignment.unique()[0]
...
def _calculate_first_step_assignments(
        alternatives: pd.Index, dms: pd.Index,
    ...
    :param alternatives_general_net_flows: Series with alternatives general
     net flows
    :param profiles: Index with profiles names
    :param profiles_general_net_flows: DataFrame with profiles general net
     flows (index: (DM, profile),
     columns: alternatives)
    :param categories: List of categories names (strings only)
    :param comparison_with_profiles: Enum CompareProfiles - indicate
     information of profiles types used
    in calculation.

    :return: Tuple with imprecise classification DataFrame and not_classified
     Dictionary
    """

    classification = pd.DataFrame(index=alternatives,
                                  columns=['worse', 'better'], dtype=str)
    not_classified = {}

    for (alternative, alternative_general_net_flow), (
            _, profiles_general_net_flows_for_alternative) in zip(
        alternatives_general_net_flows.items(),
        profiles_general_net_flows.items()):
        alternative_assignments = pd.Series(index=dms, dtype=str,
                                            name=alternative)
        for DM, DM_profiles_general_net_flows_for_alternative in \
            ...
            if comparison_with_profiles == CompareProfiles.BOUNDARY_PROFILES:
                for profile_i, profile_net_flow in enumerate(
                    ...
                    if profile_i == 0 and alternative_general_net_flow <= \
                            profile_net_flow:
                        alternative_assignments[DM] = categories[0]
                        break
                    elif profile_i == len(
                            profiles) - 1 and alternative_general_net_flow > \
                            profile_net_flow:
        ...
        classification, not_classified = _classify_alternative(
            categories,
            classification,
            not_classified,
...
def _calculate_final_assignments(alternatives_general_net_flows: pd.Series,
    ...
    classified in first step of assignment.
    Calculates distance to worse and better category based on DM's decision
     and DM's alternative profile net flows.
    Then classifies alternative to category which has smaller distance to it.
    ...
    :param comparison_with_profiles: Enum CompareProfiles - indicate
     information of profiles types used
    in calculation.
    :param classification: DataFrame with imprecise assignments (columns:
     'worse', 'better')
    :param not_classified: Dictionary with alternatives which are not
     precisely classified yet
    :param assign_to_better_class: Boolean which describe preference
    of the DMs in final alternative assignment when
    ...
    for alternative, voters in not_classified.items():
        ...
        worse_category_voters_weights = dms_weights[
            worse_category_voters].unique()
        worse_category_profile = profiles[categories.index(worse_category)]
        worse_category_profiles_general_net_flows = \
            profiles_general_net_flows.loc[
                (worse_category_voters, worse_category_profile), alternative]

        better_category_voters_weights = dms_weights[
            better_category_voters].unique()
        ...
        better_category_profiles_general_net_flows = \
            profiles_general_net_flows.loc[(better_category_voters,
                                            better_category_profile),
                                           alternative]
        ...
        if comparison_with_profiles == CompareProfiles.BOUNDARY_PROFILES:
            ...
            worse_category_distance = np.sum(
                abs(worse_category_profiles_general_net_flows -
                    alternatives_general_net_flows[alternative]) *
                worse_category_voters_weights)

            better_category_distance = np.sum(
                abs(better_category_profiles_general_net_flows -
                    alternatives_general_net_flows[alternative]) *
                better_category_voters_weights)
        ...
        if better_category_distance > worse_category_distance:
            ...
            if assign_to_better_class:
                final_classification.loc[
                    alternative, 'better'] = better_category
            else:
                final_classification.loc[
                    alternative, 'better'] = worse_category
...
def calculate_flowsort_gdss_sorted_alternatives(
        ...
        comparison_with_profiles: CompareProfiles,
        assign_to_better_class: bool = True) -> Tuple[
    ...
    :return: DataFrame with imprecise assignments (columns: 'worse',
     'better') and Series with precise assignments.
    """
    flow_sort_gdss_validation(alternatives_general_net_flows,
                              profiles_general_net_flows, categories,
                              criteria_directions, profiles_performances,
    ...
    check_dominance_condition_GDSS(profiles, profiles_performances,
                                   criteria_directions)

    first_step_assignments, not_classified = \
        _calculate_first_step_assignments(alternatives, dms,
                                          alternatives_general_net_flows,
                                          profiles,
    ...
    final_step_assignments = _calculate_final_assignments(
        alternatives_general_net_flows, profiles,
