"""
.. image:: prometheePUT_figures/M7.png
...
from core.input_validation.preference import veto_validation
...
def compute_veto(
    ...
    veto: Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]]
    if categories_profiles is None:
        # calculating veto indices for alternatives over alternatives
        partial_vet = cast(pd.DataFrame, partial_vet)
        veto = _vetoes(
            criteria,
            weights,
            strong_veto,
        ...
        partial_vet = cast(Tuple[pd.DataFrame, pd.DataFrame], partial_vet)
        veto = (
            _vetoes(
                criteria,
                weights,
    ...
    if preferences is not None:
        return (
            veto,
            partial_vet,
            pc.overall_preference(preferences, veto, profiles, decimal_place),
        )
    else:
...
def _partial_veto(
    ...
    criteria: pd.Index,
    alternatives_performances: pd.DataFrame,
    profile_performances: Optional[pd.DataFrame],
    ...
    if categories_profiles is None:
        ...
        profile_performances = cast(pd.DataFrame, profile_performances)
        return (
            _veto_deep(
                veto_thresholds=veto_thresholds,
                criteria=criteria,
