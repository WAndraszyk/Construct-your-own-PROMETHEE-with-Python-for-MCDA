"""
    .. image:: prometheePUT_figures/M28.jpg
...
from core.enums import Direction, FlowType
from core.input_validation.clustering import intervalp2clust_validation

from .M3_PrometheePreference import compute_preference_indices
from .M8_PrometheeOutrankingFlows import calculate_promethee_outranking_flows

__all__ = ["cluster_using_interval_pclust"]


def _calculate_profile_based_flows(
    ...
    :return: Tuple with pd.DataFrame with the MultiIndex("R" + alternatives,
         profiles + alternatives) as index and 'positive' and 'negative'
         columns and pd.DataFrame with the profiles as index and profiles as
         columns. First DataFrame contains the profiles-based flows of the
         profiles and the second DataFrame contains preferences between
         profiles.
    """
...
def _assign_the_alternatives_to_the_categories(
    profile_based_flows: pd.DataFrame, categories: pd.Index
    ...
    :return: Tuple with dictionary with principal categories as keys and
        ...
        clustered alternatives in list as values. The submission of two
        categories in second Dictionary represents the interval category
        between them.
    """
    ...
    for Ralternative, alternative_group_flows in profile_based_flows.groupby(level=0):
        ...
        flows_differences = (
            alternative_group_flows.iloc[:-1] - alternative_flows
        ).abs()
...
def _update_of_the_central_profiles(
    ...
    :param: principal_categories: Dictionary with principal categories as keys
         and clustered alternatives in list as values. Contains the
         alternatives assigned to the principal categories.
    ...
    for i, category in enumerate(categories):
        ...
        if len(principal_categories[category]) > 0:
            central_profiles_preferences.loc[category] = alternatives_performances.loc[
                principal_categories[category]
            ].mean()
        ...
        # principal category
            ...
            if i == 0:
                ...
                # new profile performances are randomized from specific range
                    ...
                    performances = []
                    for criterion, direction in zip(
                        central_profiles_preferences.columns, directions
                    ):
                        ...
                        if direction == Direction.MAX:  # Maximization
                            min_range = 0
                            max_range = central_profiles_preferences.iloc[1][criterion]
                        ...
                        # performance, min of all alternatives performances]
                            ...
                            min_range = central_profiles_preferences.iloc[1][criterion]
                ...
                # new profile performances are randomized from specific range
                    ...
                    for criterion, direction in zip(
                        ...
                        if direction == Direction.MAX:  # Maximization
                            min_range = central_profiles_preferences.iloc[-2][criterion]
                            max_range = alternatives_performances[criterion].max()
                        # If criterion is to be minimized, new profile
                        ...
                        # performance]
                            ...
                            max_range = central_profiles_preferences.iloc[-2][criterion]
                        performances.append(random.uniform(min_range, max_range))
            ...
            # randomized from range between the closest centroids
                ...
                for criterion, direction in zip(
                    ...
                    if direction == Direction.MAX:  # Maximization
                        min_range = central_profiles_preferences.iloc[i - 1][criterion]
                        max_range = central_profiles_preferences.iloc[i + 1][criterion]
                    else:  # Minimization
                        min_range = central_profiles_preferences.iloc[i + 1][criterion]
                        max_range = central_profiles_preferences.iloc[i - 1][criterion]
                    performances.append(random.uniform(min_range, max_range))
    ...
    for criterion, direction in zip(central_profiles_preferences.columns, directions):
        reverse = direction == Direction.MIN
        central_profiles_preferences[criterion] = sorted(
            central_profiles_preferences[criterion], reverse=reverse
        )
...
def _calculate_homogeneity_index(
    ...
    for category in categories:
        ...
        if len(alternatives_in_category) > 1:
            ...
            homogeneity_indices[category] = alternatives_preferences.loc[
                alternatives_in_category, alternatives_in_category
            ].sum().sum() / (
...
def _calculate_heterogeneity_index(
    central_profiles_preferences: pd.DataFrame, categories: pd.Index
) -> pd.Series:
    """
...
def _calculate_global_quality_index(
    homogeneity_indices: pd.Series, heterogeneity_indices: pd.Series
) -> float:
    """
...
def cluster_using_interval_pclust(
    alternatives_performances: pd.DataFrame,
    preference_thresholds: pd.Series,
    indifference_thresholds: pd.Series,
    standard_deviations: pd.Series,
    ...
    :return: Tuple with pd.Series with principal and not empty interval
        ...
        and criteria as columns and global quality index as float. The
        pd.Series shows to which cluster the alternative belongs. The
        pd.DataFrame shows performances of central profiles(centroids).
    """

    # Input validation
    intervalp2clust_validation(
        alternatives_performances,
        preference_thresholds,
        indifference_thresholds,
        standard_deviations,
    ...
    alternatives_preferences = cast(pd.DataFrame, alternatives_preferences)
    ...
    central_profiles_performances = initialize_the_central_profiles(
        alternatives_performances, categories, criteria_directions
    )

    # Calculate flows between alternatives and central profiles
    (
        profile_based_flows,
        central_profiles_preferences,
    ...
    (
        ...
        interval_categories,
    ) = _assign_the_alternatives_to_the_categories(profile_based_flows, categories)
    ...
    while iteration < max_iterations and iteration_without_change < 10:
        ...
        if (
            prev_principal_categories == principal_categories
            and prev_interval_categories == interval_categories
        ):
    ...
    homogeneity_indices = _calculate_homogeneity_index(
        principal_categories, alternatives_preferences, categories
    )
    ...
    for category, alternatives in principal_categories.items():
        clusters[category] = alternatives
