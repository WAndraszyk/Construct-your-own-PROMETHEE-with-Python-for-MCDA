"""
    .. image:: prometheePUT_figures/M18.png
...
from core.input_validation.sorting import prom_sort_validation
...
def _define_outranking_relation(
    positive_flow_a: float,
    negative_flow_a: float,
    positive_flow_b: float,
    negative_flow_b: float,
) -> str:
    """
    ...
    """
    if math.isclose(positive_flow_a, positive_flow_b) and math.isclose(
        negative_flow_a, negative_flow_b
    ):
        return "I"
    elif positive_flow_a >= positive_flow_b and negative_flow_a <= negative_flow_b:
        return "P"
...
def _check_if_all_profiles_are_preferred_to_alternative(
    ...
    alternative_positive_flow: float,
    alternative_negative_flow: float,
) -> bool:
    """
    ...
    outranking_relations = category_profiles_flows.apply(
        lambda row: _define_outranking_relation(
            row["positive"],
            row["negative"],
...
def _calculate_first_step_assignments(
    ...
    classification = pd.DataFrame(
        dtype=str, index=alternatives_flows.index, columns=["worse", "better"]
    )
    ...
    for alternative, alternative_row in alternatives_flows.iterrows():
        ...
        first_i_occurrence = (
            category_profiles.get_loc(outranking_relations.str.contains("I").idxmax())
            if outranking_relations.str.contains("I").any()
            else float("inf")
        )
        first_r_occurrence = (
            category_profiles.get_loc(outranking_relations.str.contains("R").idxmax())
            if outranking_relations.str.contains("R").any()
            else float("inf")
        )
        last_p_occurrence = (
            category_profiles.get_loc(outranking_relations.str.contains("P").idxmin())
            - 1
            if outranking_relations.str.contains("P").any()
            else float("inf")
        )

        # Assignment conditions
        if outranking_relations[-1] == "P":
            classification.loc[alternative] = categories[-1]
        elif _check_if_all_profiles_are_preferred_to_alternative(
            category_profiles_flows,
            alternative_row["positive"],
            alternative_row["negative"],
        ):
            classification.loc[alternative] = categories[0]
        elif min(first_r_occurrence, first_i_occurrence) > last_p_occurrence:
            classification.loc[alternative] = categories[last_p_occurrence + 1]
        else:
            min_idx = min(first_r_occurrence, first_i_occurrence)
            classification.loc[alternative, "worse"] = categories[min_idx]
            classification.loc[alternative, "better"] = categories[min_idx + 1]
...
def _calculate_final_assignments(
    ...
    for alternative, alternative_row in not_classified.iterrows():
        ...
        if worse_category_alternatives.empty:
            ...
            worse_category_net_outranking_flow = worse_category_alternatives.apply(
                lambda row: row["positive"] - row["negative"], axis=1
            )
            positive_distance = (
                worse_category_net_outranking_flow.map(
                    lambda x: alternative_net_outranking_flow - x
                ).sum()
        ...
        if better_category_alternatives.empty:
            ...
            better_category_net_outranking_flow = better_category_alternatives.apply(
                lambda row: row["positive"] - row["negative"], axis=1
            )
            negative_distance = (
                better_category_net_outranking_flow.map(
                    lambda x: x - alternative_net_outranking_flow
                ).sum()
...
def calculate_promsort_sorted_alternatives(
    ...
    :return: Tuple with pd.DataFrame with alternatives names as index and
        assignments as columns named (worse and better) and pd.Series objects
        with assignments as values
    """
    ...
    category_profiles = pd.DataFrame(
        directed_alternatives_performances(category_profiles, criteria_directions),
        index=category_profiles.columns,
        columns=category_profiles.columns,
    )
    ...
    first_step_assignments = _calculate_first_step_assignments(
        categories, alternatives_flows, category_profiles_flows
    )
