"""
    This class computes positive and negative outranking flows based on
    preferences.
"""
import pandas as pd
...
def _calculate_flow(preferences: Union[Tuple[pd.DataFrame, pd.DataFrame],
    ...
    :param positive: If True function returns positive outranking flow else
    returns negative outranking flow.
...
def _calculate_prometheeII_style_flow(preferences: Tuple[pd.DataFrame,
    ...
    :param preferences: Tuple of alternatives vs profiles preferences and
     profiles vs alternatives preferences.
...
def calculate_promethee_outranking_flows(
    ...
    if flow_type == FlowType.PROMETHEE_I:
        ...
        return pd.DataFrame({'positive': _calculate_flow(preferences),
                             'negative': _calculate_flow(preferences,
                                                         positive=False)
                             }, index=index)
