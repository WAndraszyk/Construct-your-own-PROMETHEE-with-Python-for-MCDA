"""
    .. image:: prometheePUT_figures/M12.jpg
...
from core.input_validation.flow import net_flows_for_multiple_DM_validation
...
def _calculate_alternatives_general_net_flows(
    ...
    alternatives_net_flows = pd.DataFrame(
        index=alternatives, columns=criteria_weights.index, dtype=float
    ).fillna(0)
    ...
    for alternatives_partial_preferences, profiles_partial_preferences in zip(
        ...
        for (criterion, criterion_preferences1), (_, criterion_preferences2,) in zip(
            ...
            for (alternative_i, alternative_i_row), (profile_j, profile_j_col,) in zip(
                ...
                alternatives_net_flows.loc[alternative_i, criterion] += (
                    alternative_i_row - profile_j_col
                ).sum()
...
def _calculate_profiles_general_net_flows(
    ...
    :param dms_profile_vs_profile_partial_preferences: pd.DataFrame with
        MultiIndex(criteria, DMs, profiles) as index and
        MultiIndex(DMs, profiles) as columns. This is partial preferences
        calculated in sum of decision maker profiles sets
    ...
    # Get DMs names
    dms = dms_profile_vs_profile_partial_preferences.index.get_level_values(1).unique()

    # Get profiles names (assuming that all DMs have the same profiles names)
    profiles = dms_profile_vs_profile_partial_preferences.index.get_level_values(
        1
    ).unique()
    ...
    for (
        ...
        for (profile_i, profile_i_row), (profile_j, profile_j_col) in zip(
    ...
    for (alternatives_partial_preferences, profiles_partial_preferences, dm,) in zip(
        ...
        for (criterion, alternatives_vs_profiles_partial_preferences), (
            ...
            # to get preferences where profiles are preferred
            # to current alternative
            for (alternative_i, alternative_i_row), (_, profile_j_col) in zip(
                alternatives_vs_profiles_partial_preferences.droplevel(0).iterrows(),
                ...
                for (_, alternative_vs_profile_partial_preference), (
                    ...
                    profiles_flows.loc[(criterion, dm, profile), alternative_i] = (
                        ...
                        - profile_vs_alternative_partial_preference
                        + profiles_vs_profiles_sum.loc[(dm, profile), criterion]
                    ) / (n_profiles + 1)
    ...
    profiles_global_net_flows = pd.DataFrame(
        index=profiles_global_net_flows_index,
        columns=alternatives,
        dtype=float,
    )
    ...
    for DM_profile, profile_criteria_net_flows in profiles_flows.groupby(level=[1, 2]):
        profiles_global_net_flows.loc[DM_profile] = (
...
def calculate_gdss_flows(
    ...
    :param dms_partial_preferences: List of tuples with pd.DataFrame
        with MultiIndex(criteria, alternatives) as index and profiles as
        columns and pd.DataFrame with MultiIndex(criteria, profiles) as index
        and alternatives as columns. Each tuple stands for one decision maker.
    :param dms_profile_vs_profile_partial_preferences: pd.DataFrame with
        MultiIndex(criteria, DMs, profiles) as index and
        MultiIndex(DMs, profiles) as columns. This is partial preferences
        calculated in sum of decision maker profiles sets
    ...
    (
        dms_alternatives_partial_preferences_iterable,
        dms_profiles_partial_preferences_iterable,
    ) = zip(*dms_partial_preferences)
    dms_profiles_partial_preferences = list(dms_profiles_partial_preferences_iterable)
    ...
    alternatives_general_net_flows = _calculate_alternatives_general_net_flows(
        alternatives,
        category_profiles,
        criteria_weights,
        dms_profiles_partial_preferences,
