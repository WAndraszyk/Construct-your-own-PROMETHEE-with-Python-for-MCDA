"""
    .. image:: prometheePUT_figures/M14.png
...
from core.input_validation.ranking import promethee_i_ranking_validation

__all__ = ["calculate_prometheeI_ranking"]


def calculate_prometheeI_ranking(
    flows: pd.DataFrame, weak_preference=False
    ...
    * INCOMPARABLE,
    * WEAK_PREFERENCE,
    * REVERSE_PREFERENCE (opposite to PREFERENCE).
    ...
    for alternative_a in alternatives:
        for alternative_b in alternatives:
            ...
            if weak_preference:
                if (
                    positive_flow[alternative_a] >= positive_flow[alternative_b]
                    ...
                    pairs.append(
                        (
                            alternative_a,
                            alternative_b,
                            RelationType.REVERSE_PREFERENCE,
                        )
                    )
            else:
                if math.isclose(
                    positive_flow[alternative_a],
                    positive_flow[alternative_b],
                    rel_tol=1e-6,
                ) and math.isclose(
                    negative_flow[alternative_a], negative_flow[alternative_b]
                ):
                    pairs.append(
                        (
                            alternative_a,
                    ...
                    pairs.append(
                        (alternative_a, alternative_b, RelationType.PREFERENCE)
                    )
                elif (
                    positive_flow[alternative_a] > positive_flow[alternative_b]
                    and negative_flow[alternative_a] > negative_flow[alternative_b]
