...
def _calculate_spaces_between_criteria_ranks(criteria_ranks: pd.Series
                                             ) -> pd.Series:
    """
    ...
    for i, (criterion, rank) in enumerate(sorted_criteria_ranks[:-1].items()):
        spaces_between_criteria_ranks[criterion] = \
            sorted_criteria_ranks[i + 1] - rank
...
def _calculate_non_normalized_weights(criteria_ranks: pd.Series,
    ...
    :param criteria_ranks: pd.Series with criteria as index and ranks as
    values
    :param criteria_weight_ratio: integer or float, the ratio of
    ...
    for i, (criterion, rank) in enumerate(
            criteria_ranks.sort_values().items()):
        cumulative_ranks[criterion] = \
            rank + sum((spaces_between_criteria_ranks + 1)[:i])

    # Calculate non-normalized weights
    for i, (criterion, rank) in enumerate(cumulative_ranks.items()):
        non_normalized_weights[criterion] = \
            round(1 + (criteria_weight_ratio - 1)
...
def _calculate_correction_ratios(normalized_weights: pd.Series,
    ...
    positive_ratios = weights_df.apply(
        lambda row: (10 ** - decimal_place -
                     (row['normalized'] - row['truncated'])) / row[
                        'normalized'], axis=1)

    negative_ratios = weights_df.apply(
        lambda row: (row['normalized'] - row['truncated']) / row[
            'normalized'], axis=1)
...
def _round_properly_weights(criteria_ranks: pd.Series,
    ...
    # Calculate positive and negative ratios for final rounding
    ratios = _calculate_correction_ratios(normalized_weights,
    ...
    positive_ratios_larger_than_negative_ratios = ratios[
        ratios['positive'] > ratios['negative']].index

    # Calculate size of upward rounded set
    size_of_upward_rounded_set = int(
        _calculate_size_of_upward_rounded_set(truncated_weights,
                                              decimal_place))
    ...
    if size_of_upward_rounded_set + len(
            positive_ratios_larger_than_negative_ratios) <= n_criteria:
        ...
        if to_add_to_downward_list > 0:
            for criterion in negative_ratios.index:
                if criterion not in \
                    ...
                    round_downward_list = \
                        round_downward_list.append(pd.Index([criterion]))
    ...
    rounded_downward = normalized_weights[
        normalized_weights.index.isin(round_downward_list)].map(
        lambda weight: floor(weight * base) / base)

    # Round upward weights
    rounded_upward = normalized_weights[
        normalized_weights.index.isin(round_upward_list)].map(
        lambda weight: ceil(weight * base) / base)

    # Create Series with all criteria
    balanced_normalized_weights = pd.concat(
        [rounded_downward, rounded_upward]).reindex(criteria_ranks.index)

    return balanced_normalized_weights


def calculate_srf_weights(criteria_ranks: pd.Series,
                          criteria_weight_ratio: NumericValue,
    ...
    truncated_weights = _truncate_normalized_weights(normalized_weights,
                                                     decimal_place)

    # Round weights with special algorithm to avoid rounding errors
    balanced_normalized_weights = _round_properly_weights(normalized_weights,
                                                          normalized_weights,
                                                          truncated_weights,
                                                          decimal_place)

    # Return weights with original index order
    return balanced_normalized_weights.reindex(original_index)
