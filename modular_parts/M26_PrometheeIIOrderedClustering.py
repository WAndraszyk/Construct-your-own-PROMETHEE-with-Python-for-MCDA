"""
.. image:: prometheePUT_figures/M26.png
...
from core.clusters_commons import (
    calculate_new_profiles,
    group_alternatives,
    initialize_the_central_profiles,
)
from core.enums import FlowType
from core.input_validation.clustering import promethee_II_ordered_clustering_validation
...
from .M9_NetOutrankingFlow import calculate_net_outranking_flows

__all__ = ["promethee_II_ordered_clustering"]


def promethee_II_ordered_clustering(
...
def _sort_alternatives_to_categories(
    ...
    profiles_preference = cast(pd.DataFrame, profiles_preference)

    # calculating Net Outranking Flow
    profile_based_flows = calculate_promethee_outranking_flows(
        alternatives_preference, FlowType.PROFILE_BASED, profiles_preference
    )
    promethee_ii_flows = cast(
        pd.DataFrame, calculate_net_outranking_flows(profile_based_flows, True)
    )
    ...
    # sorting alternatives into categories
    assignments = _calculate_flowsort_assignment(categories, promethee_ii_flows)

    # if the category is empty, the algorithm force one alternative to
    # belong to it
    assignments = _force_alternative_to_empty_category(
        assignments, central_profiles.index
    )
    return assignments


def _calculate_new_profiles_mean(
    ...
    """
    profiles = calculate_new_profiles(
        profiles_performances, alternatives_performances, assignment, np.mean
    )
    profiles.fillna(0, inplace=True)
    return cast(pd.DataFrame, profiles.apply(lambda x: x.sort_values().values))


def _force_alternative_to_empty_category(
    assignments: pd.Series, categories: pd.Index
) -> pd.Series:
    """
...
def _calculate_flowsort_assignment(
    categories: pd.Index, promethee_ii_flows: pd.DataFrame
) -> pd.Series:
    """
    ...
    for Ralternative, alternative_group_flows in promethee_ii_flows.groupby(level=0):
        ...
        category_pos = np.argmin(
            np.abs(profiles_flows["net"].values - alternative_flows["net"])
        )
