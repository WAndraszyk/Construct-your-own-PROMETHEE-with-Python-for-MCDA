"""
    .. image:: prometheePUT_figures/M19.png
...
from core.input_validation.sorting import promethee_tri_validation
...
def _calculate_criteria_net_flows(
    ...
    :param alternatives_partial_preferences: Tuple with pd.DataFrame with
        alternatives names as index and profiles names as columns and
        pd.DataFrame objects with profiles names as index and alternatives
        names as columns
    ...
    :return: Tuple with pd.DataFrame with profiles names as index and criteria
        names as columns and pd.DataFrame objects with alternatives names as
...
def _calculate_deviations(
    ...
    deviations = pd.DataFrame(
        columns=profiles_criteria_net_flows.index,
        index=alternatives,
        dtype=float,
    )
    ...
    for (
        alternative,
        alternative_row,
    ) in alternatives_criteria_net_flows.iterrows():
        for profile, profile_row in profiles_criteria_net_flows.iterrows():
            ...
            if use_marginal_value:
                deviations.loc[alternative, profile] = (
                    criteria_weights * abs(alternative_row - profile_row)
                ).sum()
            else:
                deviations.loc[alternative, profile] = (
                    criteria_weights * (alternative_row - profile_row)
                ).sum()
...
def calculate_prometheetri_sorted_alternatives(
    ...
    ) = _calculate_criteria_net_flows(
        alternatives_partial_preferences, profiles_partial_preferences
    )

    deviations = _calculate_deviations(
        alternatives,
        criteria_weights,
        profiles_criteria_net_flows,
        alternatives_criteria_net_flows,
