"""
    .. image:: prometheePUT_figures/M20.png
...
from core.input_validation.sorting import flow_sort_i_validation
...
def _append_to_classification(
    ...
    optimistic_category: str,
    alternative_name: Hashable,
) -> None:
    """
    ...
    if pessimistic_category_index > optimistic_category_index:
        pessimistic_category, optimistic_category = (
            optimistic_category,
...
def _limiting_profiles_sorting(
    ...
    classification = pd.DataFrame(
        index=alternatives_flows.index, columns=["worse", "better"], dtype=str
    )
...
def calculate_flowsortI_sorted_alternatives(
    ...
    if comparison_with_profiles == CompareProfiles.LIMITING_PROFILES:
        ...
        return _central_profiles_sorting(
            categories,
            category_profiles,
            alternatives_flows,
            category_profiles_flows,
