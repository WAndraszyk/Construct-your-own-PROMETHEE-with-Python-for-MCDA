"""
.. image:: prometheePUT_figures/M25.jpg
...
from core.input_validation.clustering import ordered_clustering_validation
...
def group_into_ordered_clusters(preferences: pd.DataFrame, k: int) -> pd.Series:
    ...
    clusters_fin.index = pd.Index([f"C{i}" for i in range(1, len(clusters_fin) + 1)])
    return clusters_fin


def _search_max(preferences: List[List[float]]) -> Tuple[float, int, int]:
    """
