...
from modular_parts.sorting.M21_FlowSortII import calculate_flowsortII_sorted_alternatives
...
def promethee_II_ordered_clustering(alternatives_performances: pd.DataFrame,
    ...
    while not sorted.equals(sorted_old):
        sorted_old = sorted
        central_profiles = _calculate_new_profiles_median(central_profiles, alternatives_performances, sorted)
        sorted = _sort_alternatives_to_categories(alternatives_performances, preference_thresholds,
...
def _sort_alternatives_to_categories(alternatives_performances, preference_thresholds, indifference_thresholds,
                                     standard_deviations, generalized_criteria, directions, weights, central_profiles,
                                     categories, which_profile):
    """
    Calculates new partial preferences, prometheeII_flows, applies flowsortII and redefines new clusters.

    :return: Alternatives assignment

    """
    alternatives_preference, _ = compute_preference_indices(alternatives_performances, preference_thresholds,
                                                            indifference_thresholds, standard_deviations,
                                                            generalized_criteria, directions, weights, central_profiles)
    profiles_preference, _ = compute_preference_indices(central_profiles, preference_thresholds,
                                                        indifference_thresholds, standard_deviations,
...
def _force_alternative_to_empty_category(sorted: pd.Series, categories: pd.Index) -> pd.Series:
    """
    In case given category disappear (is empty) during the execution of the algorithm, method force randomly chosen 
